...
export const Header = () => {
  const [nameProject, setNameProject] = useState('Gemini Space')
  useEffect(() => {
    document.addEventListener('resize', () => {
      if(document.documentElement.clientWidth <= 1000) setNameProject('GS')
      if(document.documentElement.clientWidth > 1000) setNameProject('Gemini Space')
    })
  },[])
  return (
    <header className={s.header}>
      <div className={s.header_container}>
        <button className={s.burger_menu}>
          <svg className="ham3" viewBox="0 0 100 100" width="80">
            {/*onClick="this.classList.toggle('active') up*/}
            <path
              className="line top"
